...
const METER_BLOCKS = [
  ...Array.from({ length: 26 }, (_, i) => `Bloco ${String(i + 1).padStart(2, '0')}`),
  'Área Comum 01',
  'Área Comum 02'
];
const newBlock = (title = '') => ({ id: newId(), title, photos: [], gas: '', water: '' });
const newMeterGroups = () => METER_BLOCKS.map(title => newBlock(title));
const defaultSettings = () => ({ watermark: true, template: 'two', company: '', sealConfig: DEFAULT_SEAL_CONFIG });
...
function ensureReportShape(report) {
  ...
  report.groups = METER_BLOCKS.map((title, index) => {
...
function renderBlocks() {
  ...
  $('evidenceBlocks').innerHTML = currentReport.groups.map((group, groupIndex) => `
    <article class="evidence-block meter-block">
      ...
      <div class="meter-readings">
        <label>Leitura GAS<input class="meter-input" data-reading="gas:${group.id}" inputmode="decimal" type="text" placeholder="Leitura do gás" value="${escapeHtml(group.gas)}"></label>
        <label>Leitura ÁGUA<input class="meter-input" data-reading="water:${group.id}" inputmode="decimal" type="text" placeholder="Leitura da água" value="${escapeHtml(group.water)}"></label>
      </div>
      <div class="capture-actions">
        <label class="capture-button primary"><input class="photo-input" data-group="${group.id}" data-source="camera" type="file" accept="image/*" capture="environment" multiple>⌾ Tirar foto</label>
        <label class="capture-button"><input class="photo-input" data-group="${group.id}" data-source="gallery" type="file" accept="image/*" multiple>▧ Galeria</label>
      </div>
  ...
  document.querySelectorAll('.photo-input').forEach(input => input.onchange = event => {
    addFiles(event.target.dataset.group, event.target.dataset.source, event.target.files);
    event.target.value = '';
  });
...
function header(doc, page, title) { doc.setFillColor(18, 48, 71); doc.rect(0, 0, 210, 22, 'F'); doc.setTextColor(255); doc.setFontSize(15); doc.text(settings().company || 'Blexo-Check', 12, 14); doc.setFontSize(9); doc.text(`RELATÓRIO FOTOGRÁFICO · ${page}`, 198, 14, { align: 'right' }); doc.setTextColor(30, 46, 56); doc.setFontSize(18); doc.text(title, 12, 35); }
...
$('reportsButton').onclick = openReports; $('reportsButtonInline').onclick = openReports; $('settingsButton').onclick = () => $('settingsDialog').showModal(); document.querySelectorAll('[data-close]').forEach(button => button.onclick = () => $(button.dataset.close).close()); $('settingsDialog').addEventListener('close', () => { if ($('settingsDialog').returnValue !== 'cancel') { syncFields(); renderBlocks(); scheduleSave(); } }); $('generateButton').onclick = generatePdf;
