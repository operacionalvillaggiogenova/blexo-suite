...
function buildPdf(pages){
  ...
  for(let i=0;i<pages.length;i++){
    ...
    let top=H-75; const identification=i===0?pages.identification:[];
...
async function generatePdf(){
  ...
  try{
    ...
    prepared.identification=[['Empresa / condomínio',currentDocument.company],['Referência',currentDocument.reference],['Responsável',currentDocument.responsible]].filter(([,v])=>String(v||'').trim()).map(([a,b])=>[a,String(b)]);
